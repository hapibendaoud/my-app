...
useEffect(() => { 
    ...
    try {
        const response = await fetch(`${URL}/api/patients/myappointments`, {
        method: "GET",
        headers: {
...
const handleBookAppointment = async (e) => {
    ...
    if (addedAppt.visitType !== "" && addedAppt.appointmentDate !== "" && addedAppt.appointmentTime !== "") {
        try {
            const response = await fetch(`${URL}/api/patients/appointment`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${Cookies.get("token")}`,
            ...
            const data = await response.json(); // كيقرا الـ JSON الأول هنا

            if (response.ok) {
                // 🔥 الإصلاح هنا: كنجيبو الـ appointment الحقيقي اللي وسط الـ الـ response
                const savedApptFromDb = data.appointment; 
                
                // دابا غادي يتزاد بالـ _id الصحيح ديالو وميوقعش مشكل الـ key
                setAppointments([savedApptFromDb, ...appointments]);
                
                setShowForm(false);
                setNewAppt({ visitType: "", reason: "", appointmentDate: "", appointmentTime: "" });
            } else {
                // يلا الباكيند صيفط 400 (مثلا الموعد محجوز ديجا)، غايبان هاد الـ alert للمريض
